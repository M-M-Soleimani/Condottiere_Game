...
std::string Matarsak::perform_Action(Player &player)
{
    UI ui;
    ui.Display_Message("Enter a card to return it to your hand: ");
    std::string choice = ui.Get_User_Input<std::string>();

    std::vector<Card *> &played_cards = player.Get_played_crads(); // Get a reference to the vector
    ...
    ui.Display_Message("Card not found in played cards !");
    return "Card not found in played cards !";
}

// To handle the absence of errors due to overload and override
std::string Matarsak::perform_Action(std::vector<Card *> &playerCard) {}
